      ...
      {processedParentComments.map((comment) => {
        ...
        return (
          <div key={comment.id}>
            <CommentItem>
              <Left>
                <AvatarStyled>{comment.User?.nickname?.[0] || 'U'}</AvatarStyled>
                <Content>
                  <NicknameDateWrapper>
              ...
              <ReplyWrapper>
                ...
                return (
                  <CommentItem key={recomment.id} style={{ padding: '10px 16px', marginBottom: '8px', background: '#f9f9f9', borderRadius: '6px' }}>
                    <Left>
                      <AvatarStyled>{recomment.User?.nickname?.[0] || 'U'}</AvatarStyled>
                      <Content>
                        <RecommentNicknameDate>
                          <RecommentNickname>{recomment.User?.nickname || '알 수 없음'}</RecommentNickname>
