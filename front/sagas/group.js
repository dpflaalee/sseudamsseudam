...
import {
  LOAD_GROUPS_REQUEST, LOAD_GROUPS_SUCCESS, LOAD_GROUPS_FAILURE,
  ...
  KICK_MEMBER_REQUEST, KICK_MEMBER_SUCCESS, KICK_MEMBER_FAILURE,
  TRANSFER_OWNERSHIP_REQUEST, TRANSFER_OWNERSHIP_SUCCESS, TRANSFER_OWNERSHIP_FAILURE,
  APPLY_GROUP_REQUEST, APPLY_GROUP_SUCCESS, APPLY_GROUP_FAILURE,
  LOAD_JOIN_REQUESTS_REQUEST, LOAD_JOIN_REQUESTS_SUCCESS, LOAD_JOIN_REQUESTS_FAILURE,
  APPROVE_JOIN_REQUEST, APPROVE_JOIN_REQUEST_SUCCESS, APPROVE_JOIN_REQUEST_FAILURE,
  REJECT_JOIN_REQUEST, REJECT_JOIN_REQUEST_SUCCESS, REJECT_JOIN_REQUEST_FAILURE,
} from '../reducers/group';
...
function* loadGroups() {
  try {
    ...
    yield put({ type: LOAD_GROUPS_FAILURE, error: err.response.data });
  }
}
...
function updateGroupAPI(data) {
  return axios.patch(`/api/groups/${data.groupId}`, data);
}
...
function* updateGroup(action) {
  try {
    ...
    yield put({ type: UPDATE_GROUP_FAILURE, error: err.response.data });
  }
}
...
function* deleteGroup(action) {
  try {
    ...
    yield put({ type: DELETE_GROUP_FAILURE, error: err.response.data });
  }
}
